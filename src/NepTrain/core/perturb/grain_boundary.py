...
def generate_csl_grain_boundary(
    element: str,
    sigma: int,
    axis: np.ndarray,
    angle: float,
...
def test_all_crystal_systems():
    ...
    test_data = {
        'cubic': {
            'element': 'Al',
            'cell': [[4.05, 0, 0], [0, 4.05, 0], [0, 0, 4.05]],  # FCC Al
            'positions': [[0, 0, 0]],
            'symbols': ['Al'],
            'size': (2, 2, 2)
        },
        'hexagonal': {
            'element': 'Mg',
            'cell': [[3.21, 0, 0], [-1.605, 2.78, 0], [0, 0, 5.73]],  # HCP Mg
            'positions': [[0, 0, 0], [0.333, 0.667, 0.25]],
            'symbols': ['Mg', 'Mg'],
            'size': (2, 2, 2)
        },
        'tetragonal': {
            'element': 'Sn',
            'cell': [[4.73, 0, 0], [0, 4.73, 0], [0, 0, 5.83]],  # Tetragonal Sn
            'positions': [[0, 0, 0]],
            'symbols': ['Sn'],
            'size': (2, 2, 2)
        },
        'orthorhombic': {
            'element': 'S',
            'cell': [[10.48, 0, 0], [0, 12.88, 0], [0, 0, 5.48]],  # Orthorhombic S
            'positions': [[0, 0, 0]],
            'symbols': ['S'],
            'size': (2, 2, 2)
        },
        'trigonal': {
            'element': 'Bi',
            'cell': [[4.13, 0, 0], [-2.065, 3.57, 0], [0, 0, 6.65]],  # Rhombohedral Bi
            'positions': [[0, 0, 0]],
            'symbols': ['Bi'],
            'size': (2, 2, 2)
        },
        'monoclinic': {
            'element': 'S',
            'cell': [[10.48, 0, 0], [0, 12.88, 0], [0, 0, 5.48]],  # Simplified monoclinic
            'positions': [[0, 0, 0]],
            'symbols': ['S'],
            'size': (2, 2, 2)
        },
        'triclinic': {
            'element': 'Ca',
            'cell': [[5.58, 0, 0], [0.58, 6.12, 0], [0.58, 0.62, 7.25]],  # Triclinic Ca
            'positions': [[0, 0, 0]],
            'symbols': ['Ca'],
            'size': (2, 2, 2)
        },
    ...
    for crystal_sys, data in test_data.items():
        try:
            # 手动创建Atoms对象
            atoms = Atoms(
            ...
            # 检测晶系
            detected = detect_crystal_system(atoms.cell)
            
            # 测试晶界生成
            generator = GrainBoundaryGenerator(atoms)
            
            # 测试扭转晶界
            boundary = generator.generate_twist_boundary(
                rotation_axis=[0, 0, 1],
                twist_angle=15.0,
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("Testing Grain Boundary Generator for All Crystal Systems")
    print("=" * 60)
    
    # 测试立方晶系
    print("\n1. Cubic System (Al, FCC):")
    atoms_fcc = bulk('Al', cubic=True)
    generator = GrainBoundaryGenerator(atoms_fcc)
    
    print(f"   Detected crystal system: {generator.crystal_system}")
    
    # 生成.twist晶界
    gb_twist = generator.generate_twist_boundary(
        rotation_axis=[0, 0, 1],
        twist_angle=15.0,
        size=(2, 2, 2)
    )
    print(f"   Twist boundary created: {len(gb_twist)} atoms")
    
    # 生成倾转晶界
    gb_tilt = generator.generate_tilt_boundary(
        rotation_axis=[1, 0, 0],
        tilt_angle=15.0,
        boundary_plane=[0, 0, 1],
        size=(2, 2, 2)
    )
    print(f"   Tilt boundary created: {len(gb_tilt)} atoms")
    
    # 测试六方晶系
    print("\n2. Hexagonal System (Mg, HCP):")
    atoms_hcp = bulk('Mg', hexagonal=True)
    generator_hcp = GrainBoundaryGenerator(atoms_hcp)
    
    print(f"   Detected crystal system: {generator_hcp.crystal_system}")
    print(f"   Lattice parameters: {generator_hcp.get_crystal_system_info()['lattice_parameters']}")
    
    # 查看所有晶系测试结果
    print("\n" + "=" * 60)
    print("Testing All Crystal Systems:")
    print("=" * 60)
    results = test_all_crystal_systems()
    
    for system, result in results.items():
        if result['test'] == 'PASS':
            print(f"  ✓ {system:15s}: {result['num_atoms']} atoms, {result['boundary_type']} boundary")
        else:
            print(f"  ✗ {system:15s}: {result.get('error', 'Unknown error')}")
