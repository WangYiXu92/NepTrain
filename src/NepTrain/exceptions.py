...
class PerturbationError(NepTrainError):
    """
    Base exception for perturbation-related errors.
    
    This exception provides enhanced error messages with context about
    the perturbation operation that failed.
    """
    
    def __init__(self, message: str, operation: str = None, 
                 params: Dict[str, Any] = None, suggestion: str = None):
        """
        Initialize perturbation error.
        
        Args:
            message: Error message
            operation: Name of the perturbation operation that failed
...
class DimensionMismatchError(PerturbationError):
    """Raised when Sobol dimension count doesn't match."""
    
    def __init__(self, allocated: int, consumed: int, operation: str = None):
        """Initialize dimension mismatch error."""
        msg = (f"Sobol dimension mismatch: allocated {allocated}, "
               f"consumed {consumed}")
        super().__init__(msg, operation=operation,
                        suggestion="Check your perturbation parameters and "
                                  "sampler configuration")


class AtomicOverlapError(PerturbationError):
    """Raised when atoms overlap after perturbation."""
    
    def __init__(self, min_distance: float, threshold: float, 
                 operation: str = None):
        """Initialize atomic overlap error."""
        msg = (f"Atomic overlap detected: min_distance={min_distance:.3f}, "
               f"threshold={threshold:.3f}")
        super().__init__(msg, operation=operation,
                        suggestion="Reduce perturbation magnitude or "
                                  "increase min_distance parameter")


class TopologyError(PerturbationError):
    """Raised when topology modification fails."""
    
    def __init__(self, operation: str, details: str = None):
        """Initialize topology error."""
        msg = f"Topology modification failed: {operation}"
        if details:
            msg += f": {details}"
        super().__init__(msg, operation=operation,
                        suggestion="Check input parameters for validity")


class MemoryLimitError(PerturbationError, MemoryError):
    """Raised when memory limit is exceeded."""
    
    def __init__(self, requested: int, available: int = None, 
                 operation: str = None):
        """Initialize memory limit error."""
        msg = f"Out of memory: requested {requested} elements"
        if available:
            msg += f", available {available}"
        super().__init__(msg, operation=operation,
                        suggestion="Reduce batch size or structure size")
